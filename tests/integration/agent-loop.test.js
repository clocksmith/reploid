...
describe('AgentLoop - Integration Tests', () => {
  ...
  const createMocks = () => {
    ...
    mockToolRunner = {
      list: () => ['tool1', 'tool2', 'tool3', 'tool4', 'tool5', 'tool6', 'tool7', 'failing_tool', 'test', 'ReadFile', 'WriteFile', 'ListFiles', 'CreateTool', 'LoadModule', 'Promote', 'DeleteFile', 'KatamariEngine', 'AwaitWorkers'],
      execute: vi.fn()
    };
  ...
  describe('initialization', () => {
    it('keeps model and strategy selection fixed during an active attempt', async () => {
      agentLoop.setModel({ id: 'fixture', provider: 'test' });
      mockLLMClient.chat.mockResolvedValue({ content: 'DONE' });
      mockResponseParser.isDone.mockReturnValue(true);
      const pending = agentLoop.run('Inspect the bounded goal');
      expect(() => agentLoop.setModel({ id: 'other' })).toThrow('Pause');
      expect(() => agentLoop.setConsensusStrategy('swarm')).toThrow('Pause');
      await pending;
      expect(() => agentLoop.setConsensusStrategy('arena')).not.toThrow();
      await agentLoop.close();
    });
