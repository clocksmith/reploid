...
describe('local Doppler model contract', () => {
  it('exposes Qwen as the only local Doppler option for now', () => {
    ...
    expect(DOPPLER_PACKAGE_VERSION).toBe('0.4.9');
    expect(DOPPLER_MODULE_URL).toBe('https://esm.sh/doppler-gpu@0.4.9/src/client/doppler-api.js?bundle');
    expect(DOPPLER_KERNEL_BASE_URL).toBe('https://esm.sh/doppler-gpu@0.4.9/src/gpu/kernels');
    expect(DOPPLER_TOOLING_URL).toBe('https://esm.sh/doppler-gpu@0.4.9/tooling?bundle');
