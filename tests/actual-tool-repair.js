...
try {
  ...
  const setup = await page.evaluate(async ({ maxAttempts, diagnosticTimeoutMs }) => {
    ...
    const { default: toolPolicy } = await import('/config/work-evolution.json', { with: { type: 'json' } });
    ...
    state.promise = (async () => {
      try {
        ...
        for (let index = 0; index < maxAttempts; index++) {
          ...
          // Reproduce only the original public task input. Never expose protected workload observations.
          let diagnostic = null;
          if (candidate.evaluation) {
            try { diagnostic = { input, result: await runIsolatedCode(code, input, { signal: controller.signal,
              timeoutMs: toolPolicy.timeoutMs, maxResultBytes: toolPolicy.maxResultBytes }) }; }
            catch (error) { controller.signal.throwIfAborted(); diagnostic = { input, error: error.message }; }
          }
          state.attempts.push({ instruction, response: result, code, candidate, evidence, diagnostic });
          if (candidate.status === 'awaiting-approval') {
            state.qualified = true; state.offer = await evolution.exportOffer(candidate.id); break;
          }
          const feedback = candidate.error || 'The host comparison rejected this candidate. Repeating the original input produced: '
            + JSON.stringify(diagnostic) + '. Check the complete public contract and preserve JSON values.';
