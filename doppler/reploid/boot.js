...
/**
 * Initialize wizard and setup awaken trigger
 */
async function initWizard(bootResult) {
  ...
  // Expose awaken trigger for wizard
...
(async () => {
  try {
    // Run main boot sequence (no legacy UI dependencies)
    const bootResult = await boot(Utils, DIContainer);

    // Initialize wizard
    await initWizard(bootResult);
