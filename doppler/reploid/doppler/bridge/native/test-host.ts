 ...
 */
function createNativeMessage(obj: NativeMessage): Buffer {
 ...
 * Create PING message
 */
function createPingMessage(reqId: number): NativeMessage {
 ...
 * Create READ message
 */
function createReadMessage(reqId: number, filePath: string, offset = 0, length = 1024): NativeMessage {
 ...
 * Create LIST message
 */
function createListMessage(reqId: number, dirPath: string): NativeMessage {
 ...
 */
function parseResponse(data: NativeMessage): ParsedResponse {
  ...
  const payload = payloadLen > 0 ? buf.slice(HEADER_SIZE, HEADER_SIZE + payloadLen) : undefined;
...
async function runTests(): Promise<void> {
  ...
  const host: ChildProcess = spawn('node', [hostPath], {
    stdio: ['pipe', 'pipe', 'pipe'],
  });

  let responseBuffer = Buffer.alloc(0);
  const responses: NativeMessage[] = [];
  ...
  const sendAndWait = (msg: NativeMessage, timeout = 2000): Promise<NativeMessage> => {
  ...
  try {
    ...
    if (read.cmd === CMD_READ_RESPONSE && read.reqId === 2 && read.payload) {
    ...
    } else if (read.cmd === CMD_ERROR && read.payload) {
    ...
    if (secRead.cmd === CMD_ERROR && secRead.payload) {
    ...
    if (list.cmd === CMD_LIST_RESPONSE && list.reqId === 5 && list.payload) {
      const entries = JSON.parse(list.payload.toString('utf8')) as ListEntry[];
    ...
    } else if (list.cmd === CMD_ERROR && list.payload) {
