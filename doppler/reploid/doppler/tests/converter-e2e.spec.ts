 ...
 *
 * Run with: npx playwright test doppler/tests/converter-e2e.spec.ts --headed
 */
...
test.describe('Browser Model Converter', () => {
  ...
  test.skip('converts tiny test model and verifies RDRR output', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      (window as any)._originalPickFiles = (window as any).pickModelFiles;
      (window as any).pickModelFiles = async () => {
        return (window as any)._testFiles;
      };
    });
...
/**
 * Options for convert and generate test
 */
export interface ConvertAndGenerateTestOptions {
  modelFiles?: any;
  prompt?: string;
...
/**
 * Convert and generate test result
 */
export interface ConvertAndGenerateTestResult {
 ...
 */
export async function runConvertAndGenerateTest(
  page: Page,
  options: ConvertAndGenerateTestOptions = {}
): Promise<ConvertAndGenerateTestResult> {
  const {
    modelFiles,
    prompt = 'the sky is',
