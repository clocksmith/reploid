...
const UI = {
  metadata: {
    id: 'UI',
    version: '1.0.0',
    dependencies: ['config', 'Utils', 'Storage', 'StateManager'],
  ...
  factory: (deps) => {
    ...
    let { config, Utils, Storage, StateManager } = deps;
  ...
  const setupEventListeners = () => {
      uiRefs.runCycleButton?.addEventListener('click', async () => {
          if (!CycleLogic) {
              logger.error("CycleLogic module not available to UI Manager. Cannot run cycle.");
              updateStatus("ERROR: CycleLogic not loaded.");
              return;
          }
