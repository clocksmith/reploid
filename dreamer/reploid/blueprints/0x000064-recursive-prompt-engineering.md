...
**Affected Artifacts:** `/system/prompt-template.md`, `/system/prompt-history.json`, `/modules/prompt-evaluator.js`
...
const PromptEvolutionPipeline = {
  ...
  deployIfBetter: async (currentPrompt, variantPrompt, testResults) => {
    ...
    if (variantScore > currentScore * 1.05) { // 5% improvement threshold
      ...
      await VFS.writeFile(
        `/system/prompt-archive/v${currentPrompt.version}.json`,
      ...
      await VFS.writeFile('/system/prompt-template.json', JSON.stringify(newPrompt));
...
```javascript
// In agent-loop.js, before each cycle:
