...
export class InferencePipeline {
  constructor() {
    ...
    // Base URL for loading assets (tokenizer.json, etc.)
    this.baseUrl = null;
    ...
    // Attention kernel override (auto/tiled_large/tiled_small/streaming)
    this.attentionKernelOverride = null;
    this.manifestAttentionKernelDefault = null;

    // Debug logging (off by default)
    this.debug = false;

    // Command batching: single GPU submit per forward pass
    // Reduces JS<->GPU overhead from ~260 submits to 1 submit per forward
    // Set to false for debugging (each kernel submits independently)
    this.useBatchedCommands = true;

    // Performance stats for batched vs unbatched comparison
    this.batchingStats = {
   ...
   * @param {Object} contexts.gpu - GPU context
   * @param {Object} contexts.memory - Memory context
   * @param {Object} contexts.storage - Storage context
   * @param {string} contexts.baseUrl - Base URL for loading assets (tokenizer.json, etc.)
   */
  ...
  async loadModel(manifest) {
    ...
    let cacheLayout = cacheMaxSeqLen > 8192 ? 'paged' : 'contiguous';
    ...
    cacheConfig.kvDtype = this.useGPU && gpuCaps.hasF16 ? 'f16' : 'f32';
  ...
  _parseModelConfig(manifest) {
    const rawConfig = manifest.config || manifest.modelConfig || {};
    // For multimodal models (e.g., Gemma 3), text config is nested in text_config
    const textConfig = rawConfig.text_config || {};
    // Merge configs, preferring text_config values for text model properties
    const config = { ...rawConfig, ...textConfig };

    // Map GGUF-style field names to HuggingFace-style for compatibility
    // GGUF uses: embeddingLength, blockCount, attentionHeadCount, attentionHeadCountKV, feedForwardLength
    // HF uses: hidden_size, num_hidden_layers, num_attention_heads, num_key_value_heads, intermediate_size
    ...
    console.log('[Pipeline] Config parsing:');
    console.log('  rawConfig.hidden_size:', rawConfig.hidden_size);
    console.log('  textConfig.hidden_size:', textConfig.hidden_size);
    console.log('  config.hidden_size:', config.hidden_size);
    console.log('  textConfig.num_attention_heads:', textConfig.num_attention_heads);
    console.log('  textConfig.num_key_value_heads:', textConfig.num_key_value_heads);
    console.log('  textConfig.head_dim:', textConfig.head_dim);

    const hiddenSize = config.hidden_size || config.n_embd || 4096;

    // Try to infer attention params from tensor shapes if not in config
    ...
    // Determine vocab size from config, tokenizer, or tensor shapes.
    const vocabCandidates = [];
    const configVocab = Number(config.vocab_size || rawConfig.vocab_size);
    if (Number.isFinite(configVocab) && configVocab > 0) vocabCandidates.push(configVocab);
    const tokenizerVocab = Number(manifest.tokenizer?.vocabSize);
    if (Number.isFinite(tokenizerVocab) && tokenizerVocab > 0) vocabCandidates.push(tokenizerVocab);
    const inferredVocab = this._inferVocabSizeFromTensors(manifest, hiddenSize);
    if (Number.isFinite(inferredVocab) && inferredVocab > 0) vocabCandidates.push(inferredVocab);

    const vocabSize = vocabCandidates.length
      ? Math.max(...vocabCandidates)
      : 32000;

    // If attention params missing, try to infer from tensor shapes
    if (!numHeads || !headDim) {
      const inferred = this._inferAttentionParams(manifest, hiddenSize, numHeads);
      if (inferred) {
        numHeads = numHeads || inferred.numHeads;
        numKVHeads = numKVHeads || inferred.numKVHeads;
        headDim = headDim || inferred.headDim;
        console.log(`[Pipeline] Inferred attention params from tensors: numHeads=${numHeads}, numKVHeads=${numKVHeads}, headDim=${headDim}`);
      }
    }
    ...
    // RoPE scaling (linear supported; other types treated as linear with warning)
    const ropeScaling = config.rope_scaling;
    let ropeScale = 1.0;
    let ropeScalingType = null;
    if (ropeScaling && typeof ropeScaling === 'object') {
      ropeScalingType = ropeScaling.rope_type || ropeScaling.type || 'linear';
      const factor = Number(ropeScaling.factor || ropeScaling.scale || 1);
      if (Number.isFinite(factor) && factor > 0) {
        if (ropeScalingType !== 'linear') {
          console.warn(`[Pipeline] rope_scaling type "${ropeScalingType}" not fully supported; applying linear factor ${factor}`);
        }
        ropeScale = factor;
      }
    }

    // GPT-OSS specific: per-layer attention types
    const isGptOss = this._isGptOssModel(rawConfig, manifest);
    ...
    const rmsNormEps = config.rms_norm_eps || (this._isGemmaModel(rawConfig, manifest) ? 1e-6 : 1e-5);
    console.log('[Pipeline] Final model config:');
    console.log('  numLayers:', config.num_hidden_layers || config.n_layer || 32);
    console.log('  hiddenSize:', hiddenSize);
    console.log('  numHeads:', numHeads);
    console.log('  numKVHeads:', numKVHeads);
    console.log('  headDim:', headDim);
    console.log('  vocabSize:', vocabSize);
    console.log('  hiddenActivation:', this._normalizeActivation(config.hidden_activation || config.hidden_act || 'silu'));
    console.log('  rmsNormEps:', rmsNormEps);
    console.log('  rmsNormWeightOffset:', this._isGemma3Model(rawConfig, manifest));
    ...
    return {
      ...
      // Gemma 3 uses rope_theta=1M, most other models use 10K
      ropeTheta: config.rope_theta || (this._isGemmaModel(rawConfig, manifest) ? 1000000 : 10000),
      ropeScale,
      ropeScalingType,
      ...
      // Normalization (Gemma uses 1e-6, most others use 1e-5)
      rmsNormEps: config.rms_norm_eps || (this._isGemmaModel(rawConfig, manifest) ? 1e-6 : 1e-5),

      // Gemma 3+ uses (1 + weight) in RMSNorm instead of just weight
      // The stored weights are the offset, so we need to add 1 when loading
      rmsNormWeightOffset: this._isGemma3Model(rawConfig, manifest),

      // Model-specific features
      // Gemma models require embedding scaling by sqrt(hidden_size)
      scaleEmbeddings: this._isGemmaModel(rawConfig, manifest),

      // Hidden activation function (silu for LLaMA/Mistral, gelu for Gemma 3)
      // Normalize variants: "gelu_pytorch_tanh" -> "gelu", "silu" -> "silu"
      hiddenActivation: this._normalizeActivation(config.hidden_activation || config.hidden_act || 'silu'),

      // Gemma chat template and stop tokens
      isGemma: this._isGemmaModel(rawConfig, manifest),
      stopTokenIds: this._getStopTokenIds(rawConfig, manifest),

      // GPT-OSS specific config
      isGptOss,
      layerTypes,  // Array of 'sliding_attention' | 'full_attention' per layer
      attentionBias: config.attention_bias || false,
    };
  }

  /**
   * Check if model is Gemma family
   * @private
   */
  _isGemmaModel(config, manifest) {
   ...
   * Gemma 3 and later use a different RMSNorm formula: output = (x / rms) * (1 + weight)
   * instead of the standard: output = (x / rms) * weight
   * @private
   */
  _isGemma3Model(config, manifest) {
    const arch = manifest?.architecture || '';
    const modelType = config?.model_type || config?.text_config?.model_type || '';
    // Gemma3ForCausalLM, Gemma3nForCausalLM, etc.
    // Also check model_type for 'gemma3', 'gemma3n'
    const isGemma3 = /gemma3/i.test(arch) || /gemma3/i.test(modelType);
    console.log(`[DEBUG] _isGemma3Model: arch="${arch}", modelType="${modelType}", result=${isGemma3}`);
    return isGemma3;
  }

  /**
   * Normalize activation function name to internal format
   * @param {string} activation - Raw activation name from config
   * @returns {string} Normalized activation: 'silu' or 'gelu'
   * @private
   */
  _normalizeActivation(activation) {
    ...
    // Default to silu (most common)
    return 'silu';
  }

  /**
   * Check if model is GPT-OSS family (MoE with alternating attention)
   * @private
   */
  _isGptOssModel(config, manifest) {
    const arch = manifest?.architecture || '';
    const modelType = config?.model_type || config?.text_config?.model_type || '';
    return /gpt.?oss/i.test(arch) || /gpt.?oss/i.test(modelType);
  }
   ...
   * @private
   */
  _getStopTokenIds(config, manifest) {
    ...
    if (this._isGemmaModel(config, manifest)) {
      return [1, 106];
    }
    return [];
  }
   ...
   * Used when config is incomplete (e.g., Gemma 3 multimodal models)
   * @param {Object} manifest - Model manifest with tensor shapes
   * @param {number} hiddenSize - Hidden dimension size
   * @param {number} [knownNumHeads] - Known numHeads from config (if available)
   * @private
   */
  _inferAttentionParams(manifest, hiddenSize, knownNumHeads = null) {
    const tensors = manifest.tensors || {};

    // Look for q_proj and k_proj weight tensors to infer dimensions
    // Tensor names vary by format:
    // - HuggingFace: "layers.0.self_attn.q_proj.weight" or "language_model.model.layers.0.self_attn.q_proj.weight"
    // - GGUF: "blk.0.attn_q.weight"
    let qShape = null;
    let kShape = null;

    for (const [name, tensor] of Object.entries(tensors)) {
      if (name.includes('layers.0.self_attn.q_proj.weight') ||
          name.includes('layers.0.attention.q_proj.weight') ||
          name === 'blk.0.attn_q.weight') {
        qShape = tensor.shape;
      }
      if (name.includes('layers.0.self_attn.k_proj.weight') ||
          name.includes('layers.0.attention.k_proj.weight') ||
    ...
    // Detect by checking which dimension matches hiddenSize
    let qOutDim, kOutDim;
    if (qShape[0] === hiddenSize) {
      // GGUF format: [in, out]
      qOutDim = qShape[1]; // numHeads * headDim
      kOutDim = kShape[1]; // numKVHeads * headDim
      console.log('[Pipeline] Detected GGUF tensor shape format [in, out]');
    } else {
      // PyTorch format: [out, in]
      qOutDim = qShape[0]; // numHeads * headDim
      kOutDim = kShape[0]; // numKVHeads * headDim
    }
    ...
    // GGUF uses: "blk.0.attn_q_norm.weight"
    for (const [name, tensor] of Object.entries(tensors)) {
      if ((name.includes('layers.0.self_attn.q_norm.weight') || name === 'blk.0.attn_q_norm.weight') &&
        ...
        if (qOutDim % normHeadDim === 0 && kOutDim % normHeadDim === 0) {
          ...
          if (numHeads >= numKVHeads && numHeads > 0 && numKVHeads > 0) {
            console.log(`[Pipeline] Inferred headDim=${normHeadDim} from q_norm.weight shape`);
    ...
    // Common headDim values to try (256 first for Gemma, then LLaMA/Mistral common values)
    ...
    for (const testHeadDim of commonHeadDims) {
      if (qOutDim % testHeadDim === 0 && kOutDim % testHeadDim === 0) {
        ...
        // Sanity check: numHeads should be >= numKVHeads and both > 0
        if (numHeads >= numKVHeads && numHeads > 0 && numKVHeads > 0) {
  ...
  /**
   * Infer vocab size from embedding or lm_head tensor shapes.
   * Used when config/tokenizer metadata is missing or incomplete.
   * @private
   */
  _inferVocabSizeFromTensors(manifest, hiddenSize) {
    const tensors = manifest.tensors || {};
    let best = 0;
    ...
    for (const [name, tensor] of Object.entries(tensors)) {
      ...
      if (shape.length === 2) {
        ...
        if (Number.isFinite(a) && Number.isFinite(b)) {
          if (b === hiddenSize) candidate = a;
          else if (a === hiddenSize) candidate = b;
          else candidate = Math.max(a, b);
        }
      } else {
        const a = Number(shape[0]);
        if (Number.isFinite(a)) candidate = a;
      }

      if (Number.isFinite(candidate) && candidate > best) {
        best = candidate;
      }
    }
  ...
  async *generate(prompt, options = {}) {
    ...
    try {
      ...
      this._logLogitsSanity(prefillLogits, 'Prefill');

      // Sample first token from prefill logits
      this._applyRepetitionPenalty(prefillLogits, generatedIds, opts.repetitionPenalty);

      // DEBUG: Log logits after repetition penalty
      this._postPenaltyLogged = true;
      this._logLogitsSanity(prefillLogits, 'After RepPenalty');

      const firstToken = this._sample(prefillLogits, opts);
      ...
      const firstTokenText = this.tokenizer.decode([firstToken]);
      if (this.debug) {
  ...
  async _decodeStep(currentIds, opts) {
    ...
    if (isDebugStep) {
      this._logLogitsSanity(logits, `Decode[${this._decodeStepCount}]`);
    }
  ...
  async _embed(tokenIds) {
    ...
    // Read expected embedding offset for first token
    const firstTokenId = tokenIds[0];
    const expectedOffset = firstTokenId * hiddenSize * 4; // in bytes (f32)
    console.log(`[DEBUG] Gather: firstToken=${firstTokenId}, expectedByteOffset=${expectedOffset}`);

    // Read embedding at expected offset for first token
    if (expectedOffset + 64 <= embedGPUBuffer.size) {
      const readBuf = device.createBuffer({
      ...
      const encoder = device.createCommandEncoder();
      encoder.copyBufferToBuffer(embedGPUBuffer, expectedOffset, readBuf, 0, 64);
      device.queue.submit([encoder.finish()]);
      await readBuf.mapAsync(GPUMapMode.READ);
      const embedAtOffset = new Float32Array(readBuf.getMappedRange().slice(0));
      readBuf.unmap();
      readBuf.destroy();
      console.log(`[DEBUG] Embed at token ${firstTokenId} offset: ${Array.from(embedAtOffset.slice(0, 8)).map(v => v.toFixed(4)).join(', ')}`);
    } else {
      console.log(`[DEBUG] WARNING: expectedOffset ${expectedOffset} + 64 > embedBuffer.size ${embedGPUBuffer.size}`);
    }
    ...
    // DEBUG: Immediately verify gather output
    console.log(`[DEBUG] Gather output buffer size: ${outputBuffer.size}`);
    {
      ...
      const nonZeros = gatherOut.length - zeros;
      console.log(`[DEBUG] Gather output: zeros=${zeros}/${gatherOut.length}, first 8: ${Array.from(gatherOut.slice(0, 8)).map(v => v.toFixed(4)).join(', ')}`);
    }
  ...
  async _runExpert(layerIdx, expertIdx, input) {
    ...
    // 5. Down projection: result = W_down @ activated (transposeB for SafeTensors layout)
    const output = await runMatmul(activatedOutput, weights.down, numTokens, hiddenSize, intermediateSize, { transposeB: true });
  ...
  async _computeLogits(hiddenStates, numTokens = null) {
    ...
    // For tied embeddings, lmHead is [embeddingVocabSize, hiddenSize] and needs transposeB
    const logitsBuffer = await runMatmul(normedBuffer, lmHeadBuffer, numTokens, matmulVocabSize, hiddenSize, {
      transposeB: this.useTiedEmbeddings,
    });
  ...
  _sample(logits, opts) {
    ...
    // DEBUG: Log top candidates before sampling (first call only)
    if (!this._sampleDebugLogged) {
      this._sampleDebugLogged = true;
      const top5 = candidates.slice(0, 5).map(c => {
        const text = this.tokenizer?.decode?.([c.token]) || '?';
        return `"${text}"(${(c.prob * 100).toFixed(1)}%)`;
      }).join(', ');
      console.log(`[Pipeline] Sample candidates (after temp=${temperature}, topK=${topK}, topP=${topP}): ${top5}`);
    }
  ...
  /**
   * Log logits sanity snapshot (one-time debug)
   * @private
   */
  _logLogitsSanity(logits, label) {
    ...
    const indexed = [];
    for (let i = 0; i < logits.length; i++) {
      indexed.push({ id: i, logit: logits[i] });
    }
    indexed.sort((a, b) => b.logit - a.logit);
    const top5 = indexed.slice(0, 5);

    // Compute softmax probs for top-5
    const probs = this._softmax(new Float32Array(logits));
    const top5Str = top5.map(t => {
      const text = this.tokenizer ? this.tokenizer.decode([t.id]) : '?';
      const prob = (probs[t.id] * 100).toFixed(1);
      return `"${text}"(${prob}%)`;
    }).join(', ');
   ...
   * @private
   */
  async _debugCheckBuffer(buffer, label, numTokens) {
    const { hiddenSize } = this.modelConfig;
    const device = getDevice();
    ...
    if (buffer instanceof GPUBuffer) {
      ...
      const readBuf = device.createBuffer({
        size: Math.min(buffer.size, 4096), // Sample first 1024 floats
        usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
      });
      const encoder = device.createCommandEncoder();
    ...
    console.log(`[DEBUG] ${label}: size=${buffer.size || buffer.length}, tokens=${numTokens}, hidden=${hiddenSize}`);
    ...
    console.log(`[DEBUG] ${label}: first 8 values: ${Array.from(data.slice(0, 8)).map(v => v.toFixed(4)).join(', ')}`);
  }
  ...
  getBatchingStats() {
    ...
    return {
      ...this.batchingStats,
      avgBatchedTimeMs: batchedForwardCalls > 0 ? (totalBatchedTimeMs / batchedForwardCalls).toFixed(2) : 0,
      avgUnbatchedTimeMs: unbatchedForwardCalls > 0 ? (totalUnbatchedTimeMs / unbatchedForwardCalls).toFixed(2) : 0,
    };
  }
  ...
  printBatchingReport() {
    ...
    const lines = [
      ...
      `Avg batched time:        ${stats.avgBatchedTimeMs}ms`,
      `Avg unbatched time:      ${stats.avgUnbatchedTimeMs}ms`,
    ];
