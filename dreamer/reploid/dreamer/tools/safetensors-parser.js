...
/**
 * Read tensor data from a file handle at a specific offset
 * @param {FileHandle} fileHandle - Open file handle
 * @param {number} offset - Byte offset in file
 * @param {number} size - Number of bytes to read
 * @returns {Promise<Buffer>} Tensor data buffer
 */
async function readTensorDataFromHandle(fileHandle, offset, size) {
...
/**
 * Parse a single safetensors file (supports files > 2GB)
...
export async function parseSafetensorsFile(filePath) {
  ...
  // We only need to read the header, not the entire file
  ...
  try {
    ...
    for (const [name, info] of Object.entries(header)) {
      ...
      tensors.push({
        ...
        filePath, // For readTensorData to find the file
      });
    }

    // Close handle - readTensorData opens its own handle as needed
...
/**
 * Load tokenizer.json from HuggingFace model directory
 * This file contains the full tokenizer vocabulary and merge rules.
 * @param {string} modelDir - Model directory path
 * @returns {Promise<Object|null>} Tokenizer JSON or null
 */
...
export async function parseSafetensors(pathOrDir) {
  ...
  if (stats.isDirectory()) {
    ...
    if (format.sharded && format.indexPath) {
      ...
      parsed.tokenizerJson = await loadTokenizerJson(pathOrDir);
      return parsed;
