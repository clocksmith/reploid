 ...
 * @property {{attentionKernel?: 'auto'|'tiled_large'|'tiled_small'|'streaming'}|undefined} [optimizations]
 *   Optional Dreamer runtime preferences. `attentionKernel` can force an attention tier.
...
function validateManifest(manifest) {
  ...
  // Version validation - accept string or number (e.g., "1.0" or 1)
  ...
  if (typeof version !== 'number' || isNaN(version)) {
    ...
    // Allow minor version bumps (1.0, 1.1, etc.)
  ...
  // Architecture validation - accept object with params OR string (model class name)
  // When architecture is a string, we expect 'config' to contain the actual params
  if (manifest.architecture && typeof manifest.architecture === 'object') {
    ...
    // If no architecture object and no config, that's an error
    errors.push('Missing architecture or config field');
  }
  // If architecture is a string (like "Gemma3ForConditionalGeneration"), that's fine
  ...
  if (!Array.isArray(manifest.shards) || manifest.shards.length === 0) {
    ...
    for (let i = 0; i < manifest.shards.length; i++) {
      ...
      // Accept both blake3 (64 hex chars) and sha256 (also 64 hex chars)
...
export function parseManifest(jsonString) {
  ...
  // Normalize shard fields before validation
  // Handle variations in field naming from different writers
  if (Array.isArray(manifest.shards)) {
    let offset = 0;
    manifest.shards = manifest.shards.map((shard, i) => {
      const normalized = {
        index: shard.index ?? i,
        // Accept both fileName (camelCase) and filename (lowercase)
        filename: shard.filename || shard.fileName,
        size: shard.size,
        // Accept both blake3 and generic hash field
        blake3: shard.blake3 || shard.hash,
        // Compute offset if not present
  ...
  // Compute derived fields if missing (only when architecture is an object, not a string)
  if (manifest.architecture && typeof manifest.architecture === 'object') {
 ...
 * @param {RDRRManifest} manifest - Manifest object
 * @returns {string}
 */
