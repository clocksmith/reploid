...
WebGPU-native LLM inference engine for browser environments.
...
## 3. Loader (`loader/dreamer-loader.js`)
...
**Gemma 3 norm offset:** RMSNorm uses `(1 + weight) * x` instead of `weight * x`. Loader applies +1 offset during load for SafeTensors source (GGUF has it baked in).

---

## 4. Storage (`storage/`)

### RDRR Format (`rdrr-format.js`)
...
### convert-cli.js - Model Conversion
...
## 7. Provider API (`dreamer-provider.js`)
...
// Chat interface
const response = await dreamerChat(messages, options);
```
...
| File | Lines | Purpose |
|------|-------|---------|
| `dreamer-provider.js` | 590 | Public API, LLM client integration |
| `inference/pipeline.js` | ~2000 | Main inference orchestration |
| `inference/kv-cache.js` | ~400 | KV cache management |
| `inference/tokenizer.js` | ~300 | Tokenization wrapper |
| `inference/moe-router.js` | ~250 | MoE expert routing |
| `loader/dreamer-loader.js` | 1410 | Weight loading, dequant |
| `gpu/device.js` | ~300 | WebGPU initialization |
| `gpu/kernel-selector.js` | ~1500 | Kernel dispatch |
| `gpu/buffer-pool.js` | ~200 | Buffer pooling |
| `storage/rdrr-format.js` | ~400 | RDRR format parsing |
| `storage/shard-manager.js` | ~500 | OPFS shard management |
| `tools/quantizer.js` | ~600 | Q4_K quantization |
| `tools/convert-cli.js` | ~800 | Model conversion CLI |
...
- `docs/P2P.md` - P2P model distribution (planned)
