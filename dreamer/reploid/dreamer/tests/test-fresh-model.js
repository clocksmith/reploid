...
async function runTest() {
  ...
  try {
    ...
    const loadResult = await page.evaluate(async (modelUrl) => {
      try {
        // Import modules
    ...
    const inferenceResult = await page.evaluate(async (prompt) => {
      try {
        const pipeline = window._testPipeline;
        if (!pipeline) {
