...
async function readSafetensorsTensor(modelDir, tensorName, indices) {
  // First read the index to find which shard has the tensor
...
async function readRDRRTensor(rdrrDir, tensorName, indices) {
  // Read manifest
  const manifestPath = join(rdrrDir, 'manifest.json');
  const manifest = JSON.parse(await readFile(manifestPath, 'utf8'));

  const tensorInfo = manifest.tensors[tensorName];
  if (!tensorInfo) {
...
async function checkTensorStats(rdrrDir, tensorName) {
  const manifestPath = join(rdrrDir, 'manifest.json');
  const manifest = JSON.parse(await readFile(manifestPath, 'utf8'));
  ...
  // Show values around max if it's an outlier
  if (maxAbs > 10) {
