...
export class BundledTokenizer extends BaseTokenizer {
  constructor(config = {}) {
    ...
    this.scores = [];
    this.tokenTypes = [];
    this.type = 'bpe'; // 'bpe' or 'unigram'
    this.byteTokens = new Map();
    this.specialTokenPatterns = []; // [{content, id}, ...] for pattern matching during encode
  }
  ...
  load(tokenizerJson) {
    ...
    const isHuggingFace = tokenizerJson.model?.vocab !== undefined;

    if (isHuggingFace) {
      this._loadHuggingFaceFormat(tokenizerJson);
    } else {
      this._loadBundledFormat(tokenizerJson);
    }
  }
   ...
   * @private
   */
  _loadBundledFormat(tokenizerJson) {
  ...
  _encodeUnigram(text) {
    ...
    while (pos > 0) {
      ...
      if (state.isBytes) {
        for (let j = state.bytes.length - 1; j >= 0; j--) {
          const byteId = this.byteTokens.get(state.bytes[j]);
          tokens.push(byteId ?? this.specialTokens.unk);
        }
      } else {
        const substr = prefixed.slice(state.prev, pos);
        const tokenId = this.vocab.get(substr);
        tokens.push(tokenId ?? this.specialTokens.unk);
      }
      pos = state.prev;
    }
  ...
  _encodeBPE(text) {
    ...
    while (pos < prefixed.length) {
      let bestLen = 0;
      let bestId = this.specialTokens.unk;
      ...
      if (bestLen === 0) {
        ...
        for (const b of bytes) {
          ...
          ids.push(byteId ?? this.specialTokens.unk);
        }
        pos += 1;
      } else {
        ids.push(bestId);
...
export class Tokenizer {
   ...
   * @param {string} options.baseUrl - Base URL for loading tokenizer.json
   */
    ...
    if (isBundled && tokenizerConfig.file) {
      ...
      if (baseUrl) {
        ...
        try {
          ...
          console.warn(`[Tokenizer] Failed to fetch bundled tokenizer from URL: ${err.message}`);
        }
      } else {
        ...
        try {
          ...
          console.warn(`[Tokenizer] Failed to load bundled tokenizer from OPFS: ${err.message}`);
        }
      }
   ...
   * @param {boolean} trim - Whether to trim whitespace (default true, set false for streaming)
   * @returns {string}
   */
  decode(ids, skipSpecialTokens = true, trim = true) {
    if (!this.backend) {
