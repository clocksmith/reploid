 ...
 * @property {Function} run - Test function(page, config)
 */
...
const tests = [
    ...
    async run(page, config) {
      const result = await page.evaluate(async () => {
        try {
          ...
          const { acquireBuffer, releaseBuffer, getPoolStats } = await import('../gpu/buffer-pool.js');
          ...
          const statsAfterAcquire = getPoolStats();

          // Release
          releaseBuffer(buf1);
          releaseBuffer(buf2);

          const statsAfterRelease = getPoolStats();
    ...
    async run(page, config) {
      const result = await page.evaluate(async () => {
        try {
          ...
          // Run gather
          const outputBuffer = await runGather(tokenIdBuffer, embedBuffer, tokenIds.length, hiddenSize, vocabSize);
...

  {
    ...
    async run(page, config) {
      ...
      if (!result.success) {
        throw new Error(`Matmul test failed: ${result.error}, got ${result.output}, expected ${result.expected}`);
      }
    ...
    async run(page, config) {
      const result = await page.evaluate(async () => {
        try {
          ...
          const buf2 = recorder.createTempBuffer(512, GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST, 'temp2');
...

  {
    ...
    async run(page, config) {
      const result = await page.evaluate(async () => {
        try {
          const { log, tensor, perf, setLogLevel, getLogHistory, clearLogHistory } = await import('../debug/index.js');
      ...
      const result = await page.evaluate(async (modelUrl) => {
        try {
          ...
          const success = await downloadModel(modelUrl, (progress) => {
            if (progress.stage === 'downloading' && progress.percent % 20 === 0) {
...

  {
    ...
    async run(page, config) {
      ...
      for (const elem of modelElements) {
        ...
        if (text.toLowerCase().includes(modelName.toLowerCase()) && text.toLowerCase().includes('1b')) {
          try {
...
async function main() {
  ...
  if (config.filter) {
    testsToRun = tests.filter(t =>
      t.name.includes(config.filter) ||
      t.tags?.some(tag => tag.includes(config.filter))
    );
  }
