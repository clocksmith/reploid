 ...
 *
 * Input:
 *   - GGUF file: will be converted to .rpl via convert-cli.js
 *   - .rpl folder (contains manifest.json + shard_*.bin): served as-is
 */
...
Input:
  ...
  - .rpl folder containing manifest.json and shard_*.bin

Options:
  --port, -p <num>   Port to serve on (default: 8765)
  --output, -o <dir> Output directory for converted .rpl (default: temp)
  --keep             Keep converted .rpl directory after exit
...
Examples:
  ...
  node serve-cli.js ./model-rpl-folder --port 9000
...
async function detectInputType(inputPath) {
  ...
  if (stats.isDirectory()) {
    ...
    if (files.some(f => f === 'manifest.json')) return 'rpl';
    throw new Error(`Directory ${inputPath} does not look like an .rpl pack (missing manifest.json)`);
  }
  if (extname(inputPath).toLowerCase() === '.gguf') return 'gguf';
  throw new Error(`Unsupported input: ${inputPath} (must be .gguf or .rpl folder)`);
}

async function runConvert(inputPath, outputDir) {
  const cliPath = resolve(__dirname, './convert-cli.js');
  await mkdir(outputDir, { recursive: true });
  console.log(`[serve-cli] Converting GGUF → .rpl at ${outputDir}`);
...
async function validateRPL(dir) {
...
async function main() {
  ...
  if (inputType === 'gguf') {
    tempDir = args.output ? resolve(args.output) : path.join(os.tmpdir(), `dreamer-rpl-${Date.now()}`);
    serveDir = await runConvert(inputPath, tempDir);
  }

  await validateRPL(serveDir);
