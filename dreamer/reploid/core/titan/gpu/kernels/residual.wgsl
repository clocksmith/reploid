...
fn add_inplace(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Note: This modifies 'a' in place - 'a' must have read_write access
}
...
// Fused residual + scale (for some architectures)
@compute @workgroup_size(256, 1, 1)
fn add_scaled(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // output = a + scale * b (scale would need to be in uniforms)
    output[idx] = a[idx] + b[idx];
}
