...
- **Primary Reploid Launch**: attach a local executor, enable remote ring slots, or optionally unlock a sponsor access window
...
Most ES6 imports are served from IndexedDB. A small allowlist always stays on network, and blob-imported VFS modules do not pass through the service worker.
