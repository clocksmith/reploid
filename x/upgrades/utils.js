const UtilsModule = (() => {
  ...
  const Errors = { ApplicationError, ApiError, ToolError, StateError, ConfigError, ArtifactError, AbortError, WebComponentError };

  const logger = {
    logEvent: (level, message, ...details) => console[level] ? console[level](`[${level.toUpperCase()}] ${message}`, ...details) : console.log(`[${level.toUpperCase()}] ${message}`, ...details),
  ...
  const trunc = (str, len) => (str.length > len ? str.substring(0, len - 3) + "..." : str);
