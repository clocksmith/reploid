...
const SentinelFSM = {
  metadata: {
    id: 'SentinelFSM',
    version: '2.4.0',
    dependencies: ['StateManager', 'EventBus', 'Utils', 'SentinelTools', 'GitVFS', 'ReflectionStore', 'SelfTester', 'SwarmOrchestrator', 'HybridLLMProvider', 'VerificationManager'],
    async: false,
    type: 'service'
  },

  factory: (deps) => {
    const { StateManager, EventBus, Utils, SentinelTools, GitVFS, ReflectionStore, SelfTester, SwarmOrchestrator, HybridLLMProvider, VerificationManager } = deps;
    const { logger } = Utils;
    ...
    // Listen for manual rollback requests from UI
    EventBus.on('proposal:rollback', async () => {
      logger.warn('[SentinelFSM] Manual rollback requested');
      await rollbackChanges();
    });

    // Rollback to pre-apply checkpoint
    const rollbackChanges = async () => {
      if (!cycleContext || !cycleContext.checkpointId) {
        logger.error('[SentinelFSM] Cannot rollback: No active cycle or checkpoint');
        EventBus.emit('rollback:failed', { reason: 'No checkpoint available' });
        return false;
      }

      try {
        logger.info(`[SentinelFSM] Rolling back to checkpoint: ${cycleContext.checkpointId}`);
        updateStatusUI('APPLYING_CHANGESET', 'Rolling back changes...', null);

        const result = await StateManager.restoreCheckpoint(cycleContext.checkpointId);

        if (result.success) {
          logger.info('[SentinelFSM] Rollback successful');
          EventBus.emit('rollback:complete', { checkpointId: cycleContext.checkpointId });
          updateStatusUI('IDLE', 'Rollback complete', null);

          // Reset cycle context
          cycleContext.checkpointId = null;
          transitionTo('IDLE');
          return true;
        } else {
        ...
        logger.error('[SentinelFSM] Rollback failed:', error);
        EventBus.emit('rollback:failed', { reason: error.message });
        updateStatusUI('ERROR', `Rollback failed: ${error.message}`, null);
        return false;
      }
    };
...
Analyze the context carefully and plan your approach. When ready, use the create_dogs_bundle tool to propose specific changes.`;
    ...
    const executeApplyingChangeset = async () => {
      ...
      // PRE-FLIGHT VERIFICATION - Run proposed changes in sandboxed Worker
      if (VerificationManager && cycleContext.approvedChanges) {
        try {
          logger.info('[SentinelFSM] Running pre-flight verification in sandbox...');
          updateStatusUI('APPLYING_CHANGESET', 'Running pre-flight verification...', 50);

          const changes = cycleContext.approvedChanges.map(c => ({
            path: c.file_path,
            operation: c.operation,
            new_content: c.new_content
          }));

          const verifyResult = await VerificationManager.verifyProposal(changes);

          if (!verifyResult.passed) {
            logger.error(`[SentinelFSM] Pre-flight verification failed: ${verifyResult.reason}`);

            // Emit event for UI
            EventBus.emit('proposal:verification-failed', verifyResult);

            // Store failure in context
            cycleContext.verificationFailed = true;
            cycleContext.verificationResult = verifyResult;

            updateStatusUI('ERROR', `Verification Failed: ${verifyResult.reason}`, null);

            // Send back to planning to fix the issues
            transitionTo('PLANNING_WITH_CONTEXT');
            await executeState();
            return;
          }

          logger.info('[SentinelFSM] Pre-flight verification passed');
          cycleContext.verificationResult = verifyResult;

          // Log any warnings
          if (verifyResult.details && verifyResult.details.length > 0) {
            verifyResult.details.forEach(d => logger.info(`[Verification] ${d}`));
          }

        } catch (err) {
          logger.warn('[SentinelFSM] Verification manager error (proceeding with caution):', err);
          // Don't block on verification errors, but log them
        }
      }

      // Apply the approved changes
      updateStatusUI('APPLYING_CHANGESET', 'Applying changes...', 75);
      ...
      if (result.success) {
        // CAPTURE CHECKPOINT FOR ROLLBACK - unified checkpointing
        if (result.checkpoint) {
          cycleContext.checkpointId = result.checkpoint;
          logger.info(`[SentinelFSM] Checkpoint captured for rollback: ${result.checkpoint}`);
        }
