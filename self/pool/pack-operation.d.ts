import type { DopplerStreamAccumulator, ReploidDopplerRuntimeService } from '../infrastructure/doppler-runtime-service.js';
...
export interface PackOperationRequest {
  readonly schema: 'doppler.pack-operation-request/v1' | 'doppler.capsule-operation-request/v1' | 'doppler.capsule-operation-request/v2';
...
export interface PackOperationEventBase {
  readonly schema: 'doppler.pack-operation-event/v1' | 'doppler.capsule-operation-event/v1' | 'doppler.capsule-operation-event/v2';
...
export type PackOperationEvent = PackOperationEventBase & (
  ...
  | { readonly status: 'completed'; readonly output: JsonValue; readonly receipt: Readonly<Record<string, JsonValue>> }
);
...
export function assertPackOperationEvent(options: { binding: Readonly<Record<string, JsonValue>>; request: PackOperationRequest;
  runtimeVersion: string; event: PackOperationEvent; eventIndex: number; previousEventDigest: string | null; streamAccumulator?: DopplerStreamAccumulator | null; registry?: PackOperationRegistry }): Promise<void>;
export function runPackOperation(options: { binding: Readonly<Record<string, JsonValue>>; session: PackOperationSession; request: PackOperationRequest;
  runtimeVersion: string; runtimeService?: ReploidDopplerRuntimeService; registry?: PackOperationRegistry; signal?: AbortSignal | null; onPartial?: ((event: PackOperationEvent) => void | Promise<void>) | null;
...
export function createPackOperationStream(binding: Readonly<Record<string, JsonValue>>, request: PackOperationRequest, runtimeVersion: string, runtimeService?: ReploidDopplerRuntimeService): Promise<DopplerStreamAccumulator | null>;
