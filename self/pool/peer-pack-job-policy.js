...
export function resolvePackJobPolicy(input) {
  ...
  assert(policy?.schema === 'reploid.pool.peer-job-policy/v1' && [1, 2, 3].includes(policy.version), 'supported versioned policy required');
  ...
  if (policy.version < 3) assert(Array.isArray(policy.execution?.adapterSet) && policy.execution.adapterSet.length === 0, 'legacy unadapted execution set required');
  else {
    resolveAdapterExecutionPolicy(policy.execution?.adapters);
    assert(policy.acceptance?.schema === 'reploid.pool.operation-acceptance-policy/v1'
      && policy.acceptance.defaultMode === 'reference'
      && policy.acceptance.modes?.reference?.referenceRequired === true
      && policy.acceptance.modes.reference.operations === null
      && policy.acceptance.modes.reference.claim === 'bounded-reference-comparison'
      && policy.acceptance.modes.execution?.referenceRequired === false
      && Array.isArray(policy.acceptance.modes.execution.operations)
      && policy.acceptance.modes.execution.claim === 'execution-identity-only', 'explicit acceptance modes required');
  }
  if (policy.version >= 2) {
    assert(policy.execution.maxConcurrentJobs === 1, 'current executor supports one concurrent job');
    assert(policy.schemas.job === `reploid.peer.pack_job/v${policy.version + 1}` && policy.schemas.providerAdvert === 'reploid.peer.pack_provider/v2'
    ...
    if (policy.version === 3) assert(policy.assignmentPolicy.allowAdapterFetching === policy.execution.adapters.fetchBeforeExecute, 'adapter fetching policies disagree');
  }
  return policy;
}

export const PACK_JOB_POLICY = resolvePackJobPolicy(config.peerJobs);
assert(PACK_JOB_POLICY.version === 3, 'current runtime requires resource planning policy');
