...
/**
 * Gather Kernel - Packed BF16 Token Embedding Lookup
 *
...
@group(0) @binding(2) var<storage, read> embeddings: array<u32>;   // Packed BF16 embedding matrix
@group(0) @binding(3) var<storage, read_write> output: array<f32>; // Output [num_tokens, hidden_size]

fn load_bf16(index: u32) -> f32 {
    let packed = embeddings[index >> 1u];
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    output[tid] = load_bf16(embed_offset);
}
