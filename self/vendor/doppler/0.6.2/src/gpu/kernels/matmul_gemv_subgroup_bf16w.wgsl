// Subgroup-Optimized GEMV Kernel for packed BF16 weights
...
//
// A is f32 (activations), B is packed BF16 (weights), C is f32.
...
fn load_bf16(index: u32) -> f32 {
    let packed = B[index >> 1u];
    let bits = select(packed & 0xffffu, packed >> 16u, (index & 1u) == 1u);
    ...
    if (is_valid) {
        if (u.transpose_b == 1u) {
            ...
            for (var k_base: u32 = 0u; k_base < u.K; k_base = k_base + THREADS_PER_COL) {
                ...
                if (k < u.K) {
                    partial_sum = partial_sum + A[k] * load_bf16(b_row_offset + k);
                }
            }
        } else {
            ...
            for (; k < k_aligned_end; k = k + 4u) {
                ...
                let b0 = load_bf16(k * u.N + col);
                let b1 = load_bf16((k + 1u) * u.N + col);
                let b2 = load_bf16((k + 2u) * u.N + col);
                let b3 = load_bf16((k + 3u) * u.N + col);

                partial_sum = partial_sum + a0 * b0 + a1 * b1 + a2 * b2 + a3 * b3;
            }

            for (; k < k_end; k = k + 1u) {
                partial_sum = partial_sum + A[k] * load_bf16(k * u.N + col);
            }
        }
    }
        ...
        if (u.transpose_b == 1u) {
            ...
            for (var k4_base: u32 = 0u; k4_base < K4; k4_base = k4_base + MULTICOL_THREADS_PER_COL) {
                ...
                if (k4 < K4) {
                    ...
                    let a2 = A[k + 2u];
                    let a3 = A[k + 3u];
                    let b0 = load_bf16(b_row_offset + k);
                    let b1 = load_bf16(b_row_offset + k + 1u);
                    let b2 = load_bf16(b_row_offset + k + 2u);
                    let b3 = load_bf16(b_row_offset + k + 3u);
                    partial_sum = partial_sum + a0 * b0 + a1 * b1 + a2 * b2 + a3 * b3;
                }
            }
        } else {
        ...
        if (u.transpose_b == 1u) {
            ...
            for (var k4_base: u32 = 0u; k4_base < K4; k4_base = k4_base + THREADS_PER_COL) {
                ...
                if (k4 < K4) {
                    ...
                    let b = vec4<f32>(
                        load_bf16(b_row_offset + k),
                        load_bf16(b_row_offset + k + 1u),
                        load_bf16(b_row_offset + k + 2u),
                        load_bf16(b_row_offset + k + 3u)
                    );
            ...
            for (var k4: u32 = k4_start; k4 < k4_end; k4 = k4 + 1u) {
                ...
                let b = vec4<f32>(
                    load_bf16(k * u.N + col),
                    load_bf16((k + 1u) * u.N + col),
                    load_bf16((k + 2u) * u.N + col),
                    load_bf16((k + 3u) * u.N + col)
                );
...
fn main_cols64(
    ...
    if (is_valid) {
        ...
        if (u.transpose_b == 1u) {
            ...
            for (; k < k_end; k = k + 1u) {
                partial_sum += A[k] * load_bf16(b_row_offset + k);
            }
        } else {
            for (; k < k_aligned_end; k = k + 4u) {
                let a = vec4<f32>(A[k], A[k + 1u], A[k + 2u], A[k + 3u]);
