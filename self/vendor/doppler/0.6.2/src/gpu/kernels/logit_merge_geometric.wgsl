...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let geometric_logit = params.weight_a * logits_a[index] + params.weight_b * logits_b[index];
  merged[index] = select(geometric_logit, geometric_logit / params.temperature, params.temperature != 1.0);
}
