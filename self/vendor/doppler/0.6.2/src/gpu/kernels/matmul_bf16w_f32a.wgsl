// Matrix Multiplication Kernel - packed BF16 weights, f32 activations
//
// A is f32 (activations), B is packed BF16 (weights), C is f32.
// C[M,N] = A[M,K] * B[K,N]  (or B^T when transpose_b=1)
// BF16 values are retained exactly from source checkpoint storage and expanded
// to f32 at the point of use. Two BF16 values occupy each u32 storage element.
...
fn main(
    ...
    for (var t: u32 = 0u; t < num_tiles; t = t + 1u) {
        ...
        if (b_row < u.K && col < u.N) {
            if (u.transpose_b == 0u) {
                tileB[tile_idx] = load_bf16(b_row * u.N + col);
            } else {
                // B is [N, K], access element [col, b_row]
                tileB[tile_idx] = load_bf16(col * u.K + b_row);
            }
        } else {
            tileB[tile_idx] = 0.0;
        }
