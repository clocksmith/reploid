...
export function createCustodyContracts(ports) {
  ...
  for (const name of ["hashDopplerEvidence","validateExecutablePack","executablePacksMatch","sha256Hex","signCanonical","verifyCanonicalSignature"]) {
    if (ports[name] === undefined) throw new TypeError('Missing createCustodyContracts port: ' + name);
  }
/**
...
async function createPeerPackSupplier({ authorization, index, peerId, privateKey, inventory, readChunk, now = Date.now }) {
...
async function createPeerPackArtifactStore({ authorization, index, inventories, requesterPrivateKey, requestChunk, now = Date.now, signal, checkpoints = null, maxConcurrentChunks = 1 }) {
