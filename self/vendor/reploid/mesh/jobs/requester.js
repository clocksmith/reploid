...
export function createPackRequesterFactory(ports) {
  const { assessPeerOperation, validateOperationReference, PACK_JOB_POLICY, resolvePackJobPolicy, PEER_MESSAGE_TYPES, createPackOperationRegistry, snapshot, assertPackOperationEvent, createPackOperationStream, hashDopplerEvidence, DopplerRuntimeService, PACK_UPDATE_SCHEMA, PACK_CANCEL_SCHEMA, requirePackJob, packJobBytes, createPackPeerJob, verifyPackPeerJob, verifyPackPeerMessage, signPackPeerMessage } = ports;
  for (const name of ["assessPeerOperation","validateOperationReference","PACK_JOB_POLICY","resolvePackJobPolicy","PEER_MESSAGE_TYPES","createPackOperationRegistry","snapshot","assertPackOperationEvent","createPackOperationStream","hashDopplerEvidence","DopplerRuntimeService","PACK_UPDATE_SCHEMA","PACK_CANCEL_SCHEMA","requirePackJob","packJobBytes","createPackPeerJob","verifyPackPeerJob","verifyPackPeerMessage","signPackPeerMessage"]) {
    if (ports[name] === undefined) throw new TypeError('Missing createPackRequesterFactory port: ' + name);
  }
...
function createPackPeerRequester({ identity, bus, models, registry = createPackOperationRegistry(),
  ...
  return Object.freeze({ createPackPeerRequester });
}
