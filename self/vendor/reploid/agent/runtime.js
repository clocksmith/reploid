...
const goal = String(options.goal || '').trim();
  ...
  const run = async () => {
    ...
    while (!stopped && cycle < MAX_CYCLES) {
      ...
      let response;
      try {
        response = await wait(() => bridge.generate(snapshotJson(messages), (chunk) => {
  ...
  const checkpoint = () => {
    if (activeRun || generationRetryTimerId !== null) throw new Error('Pause execution before checkpointing');
    return snapshotJson({ schema: 'reploid.agent-checkpoint/v1', instanceId, goal, environment,
