...
export function getProviderRetryAfterMs(error, now = Date.now()) {
  ...
  const match = String(error?.message || error || '').match(/retry in\s+([0-9]+(?:\.[0-9]+)?)\s*(ms|milliseconds?|s|secs?|seconds?|m|mins?|minutes?)?/i);
...
export function isTransientProviderFailure(error) {
  ...
  return /quota|rate[-\s]?limit|too many requests|resource exhausted|retry in/i.test(String(error?.message || error || ''));
}
