# Blueprint 0x00007C: Hot-Swappable Neural Compiler

**Classification:** Canonical Full Specification

**Implementation Status:** In-Progress

**Verified Artifacts:** `/docs/trained-adapter-promotion.md`, `/self/experimental/intelligence/neural-compiler.js`, `/tests/unit/trained-adapter-admission.test.js`

**Planned Artifacts:** `/self/capabilities/validation/ui-validator.js`

**Owned Source Files:** `experimental/intelligence/neural-compiler.js`

**Former Blueprint Paths:** `self/blueprints/0x00007C-hot-swappable-neural-compiler.md`, `self/blueprints/0x00007C-hot-swappable-neural-compiler.md`
...
- 0x000066 (Arena Harness)
- 0x000041 (Module Widget Protocol)
- 0x00005F (Semantic Memory)

**Affected Artifacts:** `/experimental/intelligence/neural-compiler.js`, `/core/llm-client.js`, `/capabilities/validation/ui-validator.js`
...
## Governed externally trained adapter promotion

The compact `experimental/intelligence/neural-compiler.js` shell contained one
unique implemented contract and is merged here rather than discarded.
Externally trained adapters use a path separate from ordinary local adapter
registration:

```text
Doppler trainer identity and browser parity
  + Gamma selection receipt
  + exact base-model and adapter artifact identity
  -> stageTrainedAdapter()
  -> Shadow registry state
  -> always-require-human HITL request
  -> hash-bound approval receipt
  -> promoted load eligibility
```

Invariants:

- A staged trained adapter cannot load.
- Doppler, Gamma, manifest, base-checkpoint, tokenizer, or human-approval hash
  drift fails closed.
- Human rejection leaves the adapter staged and inactive.
- Promotion does not publish the adapter; publication is a separately signed
  Poolday operation.
- Every later activation revalidates the approval receipt against the admitted
  artifact chain.

The implementation is in `/self/experimental/intelligence/neural-compiler.js`;
its evidence and rejection behavior are covered by
`/tests/unit/trained-adapter-admission.test.js` and
`/docs/trained-adapter-promotion.md`.
