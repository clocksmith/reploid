 ...
 * Module paths in genesis-levels.json are relative to self/ (e.g., "core/vfs.js").
 * Since this file is in self/boot-helpers/, we need to go up one level.
 * @param {string} filePath - Relative file path from self/
...
const resolveModulePath = (filePath) => {
  ...
  // Paths are relative to self/, but we're in self/boot-helpers/, so go up one level
  return `../${filePath}`;
};
