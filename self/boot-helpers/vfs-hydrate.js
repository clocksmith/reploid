 ...
 * @param {string} file - File path from self/
...
const toWebPath = (file) => {
  ...
  const normalized = String(file || '').startsWith('/') ? String(file) : `/${String(file || '')}`;
  return new URL(toSourceWebPath(normalized), base).toString();
};
