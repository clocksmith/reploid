...
Use CreateTool for every new runtime tool in Zero. Each tool exports a deterministic \`tool.activation = { fixtures, checks: [{ name, args, expected }] }\` contract. CreateTool executes the checks, re-imports and replays them in a fresh fixture harness, requires matching transcripts, installs, loads, and writes evidence from the actual outcomes. Created tools start read-only unless their exported \`tool\` metadata declares capabilities such as \`vfs:write\`, \`tool:load\`, or \`self:write\`. Tool code exports \`tool\` metadata and an async default function, and uses injected deps instead of imports. Broader Reploid/X surfaces may expose Promote for evidence-gated /shadow to /self changes.`;
